...
Statistical analyses \cite{starnes:2015:practice} were performed in R \cite{R} using the \texttt{dplyr} and \texttt{ggplot2} packages \cite{dplyr, ggplot2}. 
...
\includesvg{fig2.svg}
\end{center}
\caption{Distance fallen ($y$) vs time squared ($t^2$) for the data in \cref{tab:1}}. 
\label{fig:2}
\end{figure}
%\begin{figure}
...
\section{Discussion}
Considering the widely accepted value for the gravitational constant is $g=\qty{9.81}{\meter\per\second\squared}$ \cite{tipler, openstax, barrons}, our estimate of \qty{9.4\pm0.3}{\meter\per\second\squared} is within 5\% error of the actual value, considering we had such a low-tech setup. 
...
The data also fit a model that is quadratic in time, e.g. \cref{eq:4}, as shown in \cref{fig:2} and \cref{fig:3}. These are consistent with \ref{tipler, openstax, galilei:1638:discorsi}; objects in free fall accelerate with uniform acceleration, at least for the case of small balls falling from a second-story classroom window or some nearby Renaissance tower. 
