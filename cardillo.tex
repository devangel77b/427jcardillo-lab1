...
Kinematics predicts that the acceleration due to gravity is a negative constant denoted by $-g$. However, since we are measuring the distance downwards, we take $g$ to be positive \cite{tipler,openstax,barrons}.
\begin{equation}
a(t) = -g 
\label{eq:1}
\end{equation}
Because acceleration is the instantaneous rate of change or first derivative of velocity, we can integrate over acceleration with respect to time to get an expression for velocity since derivatives and integrals are inverses by the Fundamental Theorem of Calculus \cite{calculus}. Therefore, integrating both sides of \cref{eq:1} with respect to time, we find that the velocity as a function of time is \cite{tipler, openstax, barrons}:
\begin{equation}
v(t) = -g t + v_0  
\label{eq:2}
\end{equation}
...
\label{eq:3}
\end{equation}
where the integration constant has been interpreted as the initial position, seeing as $y(t=0)=y_0$.
...
\subsection{Analysis}
Using our experimentally determined data, we plotted distance vs time squared, in order to linearize our data. This resulted in a graph whose slope would be equal to $g/2$, based on \cref{eq:4}. % and whose $R^2$ represents how well the data fit the predictions of kinematics. 
...
Statistical analyses \cite{starnes:2015:practice} were performed in R \cite{R} using the \texttt{dplyr} and \texttt{ggplot2} packages \cite{dplyr, ggplot2}. Data and code are available at \url{https://devangel77b/427jcardillo-lab1}. 



\section{Results}
\cref{tab:1} gives the measured times for each distance tested. The results are plotted in \cref{fig:2}, which gives $y$ as a function of $t^2$. 
...
\includesvg{fig2.svg}
\end{center}
\caption{Distance fallen ($y$) vs time squared ($t^2$) for the data in \cref{tab:1}. The slope is \num{4.7\pm0.1} (linear regression, $df=20$, $t=44.47$, $p<\num{2e-16}$). } 
\label{fig:2}
\end{figure}
%\begin{figure}
...
The slope of the regression line in \cref{fig:2} was 4.7; as a result, from \cref{eq:4}, we estimate $g=\qty{9.4\pm0.2}{\meter\per\second\squared}$. 
...
\section{Discussion}
\subsection{Results support $y=-\frac{1}{2}gt^2$}
Our results in \cref{tab:1,fig:2} are in good agreement with \cref{eq:4}. 

Also, considering the widely accepted value for the gravitational constant is $g=\qty{9.81}{\meter\per\second\squared}$ \cite{tipler, openstax, barrons}, our estimate of \qty{9.4\pm0.2}{\meter\per\second\squared} is within 5\% error of the actual value, considering we had such a low-tech setup. 
...
The data also fit a model that is quadratic in time, e.g. \cref{eq:4}, as shown in \cref{fig:2}. These are consistent with \cite{tipler, openstax, galilei:1638:discorsi}; objects in free fall accelerate with uniform acceleration, at least for the case of small balls falling from a second-story classroom window or some nearby Renaissance tower. Alternatives, such as linear (not accelerating) or higher order power laws were not supported.

\subsection{Limitations and sources of experimental error}
Throughout our experimentation, we experienced a few experimental errors. For instance, the tensile strength of our string was subpar and not suitable for the quantity of trials we conducted. As a result, due to the force of gravity, drops from larger heights built up greater acceleration, and therefore, a greater force was exerted upon the string, causing the string to break on one of our drops at \qty{5}{\meter}. 

However, our main setback was due to the string forming knots and tangling after each trial, due to the string bouncing as a result of its elasticity. These knots changed the length of the string and had to be manually fixed, which resulted in many retrials. 

Human error was a large cause of inconsistency with our expected results in the experiment, as both starting and stopping the stopwatch off time directly impacted the data we observed \cite{hetzler:2008:reliability,faux:2019:manual}. To remedy this, the rest of the group observed the drop, and if it appeared that the timing was incorrect, we would redo the trial.
...
We thank several anonymous reviewers for providing helpful comments. SS, JC, JC, BK, AS, and DP conducted the data collection. DP authored the Abstract, Introduction, and Discussion. BK and AS wrote the Methods and Materials. JC and JC performed the data analysis and produced the Results section.
